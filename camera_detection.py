#!/usr/bin/env python3
# edgeyolo_trt_gst_camera_postprocess_trt10_save_and_show.py
...
# ---------- CLI ----------
def parse_args():
    ap = argparse.ArgumentParser("EdgeYOLO TensorRT 10.13 + GStreamer Camera — show & save")
    ...
    ap.add_argument("--camera", default="usb", choices=["usb", "csi", "auto"])
    ap.add_argument("--device", default="/dev/video0", type=str)
    ...
    ap.add_argument("--gst", default="", type=str)
...
# ---------- GStreamer pipelines ----------
def gst_usb_pipeline(device="/dev/video0", width=1280, height=720, fps=30):
    # Kameran MJPEG veriyor -> image/jpeg + jpegdec şart.
    return (f"v4l2src device={device} ! "
            f"image/jpeg,width={width},height={height},framerate={fps}/1 ! "
            f"jpegdec ! videoconvert ! video/x-raw,format=BGR ! "
            f"appsink drop=true sync=false")

def gst_csi_pipeline(width=1280, height=720, fps=30, flip=0):
...
def main():
    ...
    # Camera (USB varsayılan: MJPEG pipelini)
    pipeline = args.gst or (gst_csi_pipeline(args.width, args.height, args.fps, args.flip)
                            if args.camera == "csi" else
                            gst_usb_pipeline(args.device, args.width, args.height, args.fps))
    cap = cv2.VideoCapture(pipeline, cv2.CAP_GSTREAMER)
    if not cap.isOpened():
        raise RuntimeError("Could not open camera")

    tracker = Tracker(distance_function="euclidean", distance_threshold=100, initialization_delay=3, hit_counter_max=150, past_detections_length=4)
    
    # Writer: ilk frame geldiğinde gerçek boyut/fps ile açacağız
    out_writer = None
    out_path = args.save if args.save else "output_camera.mp4"
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")

    # FPS tespiti (GStreamer bazen 0 döner, o zaman args.fps kullan)
    cap_fps = cap.get(cv2.CAP_PROP_FPS)
    if cap_fps is None or cap_fps <= 1:
        cap_fps = float(args.fps)

    win_name = "TRT10 Camera (press Q/ESC to quit)"
        ...
        if xyxy.size:
           ...
           # for (x1, y1, x2, y2), sc, cid in zip(xyxy, conf, cls_ids):
            #    color = color_for_class(int(cid))
             #   name = names[int(cid)] if 0 <= cid < len(names) else str(cid)
              #  cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), color, 2)
               # cv2.putText(frame, f"{name} {sc:.2f}", (int(x1), max(0, int(y1) - 6)),
            ...
            active_tracks = [t for t in tracked_objects if t.age == 0]
                        
            #draw_tracked_objects(frame, tracked_objects)
            #draw_tracked_boxes(frame, active_tracks,id_size=1.0,draw_labels=True,draw_box=True)
            draw_points(frame, drawables=tracked_objects, hide_dead_points=True)
            draw_boxes(frame, drawables=tracked_objects)
        # ---------- Lazy-init writer after first frame ( gerçek boyut ) ----------
        if out_writer is None:
            h_out_v, w_out_v = frame.shape[:2]
            out_writer = cv2.VideoWriter(out_path, fourcc, cap_fps, (w_out_v, h_out_v))
